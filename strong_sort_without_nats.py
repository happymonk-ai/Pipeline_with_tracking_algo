...
# env = lmdb.open('./face-detection.lmdb',
#                 max_dbs=10, map_size=int(1e9))

# # Now create subdbs for different types of person.
...
# async def face_detection_lmdb(data):
#     data_decode = data.decode('ISO-8859-1')
#     parse = json.loads(data_decode)
#     face_id_bytes = parse['id'].encode()
#      # Adding to respective DB:
#     with env.begin(write=True) as txn:
#         if (parse['type'] == "whitelist"):
#             txn.put(face_id_bytes, parse, db=whitelist_db)
#         elif (parse['type'] == "blacklist"):
#             txn.put(face_id_bytes, parse, db=blacklist_db)
#         elif (parse['type'] == "unknown"):
#             txn.put(face_id_bytes, parse, db=unknown_db)
#         elif (parse['type'] == "visitor"):
#             txn.put(face_id_bytes, parse, db=visitor_db)

#     # Iterate each DB to show the keys are sorted:
#     with env.begin() as txn:
#         for name, db in ('whitelist', whitelist_db), ('blacklist', blacklist_db), ('unknown', unknown_db), ('visitor', visitor_db):
#             print('DB:', name)
...
async def Activity(source,device_id,source_1):
            ...
            try:
                ...
                queue4 = Queue()
                queue5 = Queue()
                process = Process(target= run(vide_save_path, queue1, queue2, queue3, queue4, queue5))
                process.start()
                person_cnt = queue1.get()
                vehicle_cnt = queue2.get()
                detect_cnt = queue3.get()
                ppl_track = queue4.get()
                veh_track = queue5.get()
                avg_Batchcount_person.append(person_cnt)
                avg_Batchcount_vehicel.append(vehicle_cnt)
                detect_count.append(detect_cnt)
                track_person.append(ppl_track)
                track_vehicle.append(veh_track)
...
#     try :
#         data =(msg.data)
#         Process(target= await face_detection_lmdb(data=parse))
        
#     except TypeError as e:
#         print(TypeError," nats face image error >> ", e)
